...
const teamData = {
  "al-ain": [
    ...
    },
    {
      ...
      image: "/dr-kiran.png",  // 👈 empty string = falls back to initials
          ...
          {members.length === 0 ? (
            ...
            <div key={`${m.name}-${i}`} className="tp-card">
              <div className="tp-card-top" style={{ background: m.color }}>
                ...
                {m.image ? (
                  <>
                    <img src={m.image} alt={m.name} className="tp-card-photo" />
                    {/* Dark gradient so the info box stays readable over the photo */}
                    <div className="tp-card-photo-overlay" />
                  </>
                ) : (
